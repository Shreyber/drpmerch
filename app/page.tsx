...
export default function DRPMerchStore() {
	...
	// Эффект, который запускается после загрузки изображения
	useEffect(() => {
		const image = new Image(); // Создаем новый объект Image
		image.src = '/polenovo-dark.png'; // Указываем путь к изображению

		// Когда изображение загружено, обновляем состояние
		image.onload = () => {
			setIsImageLoaded(true); // Устанавливаем флаг, что изображение загружено
		};

		return () => {
			// Очищаем все обработчики, если компонент размонтируется
			image.onload = null;
		};
	}, []); // Эффект запускается один раз, когда компонент монтируется
